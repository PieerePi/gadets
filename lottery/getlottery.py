...
try:
    with open('lottery.json', 'r') as f:
        ...
        f.close()
    lastest_issue = 0 if len(old_data_list) == 0 else int(old_data_list[0]['issue'])
...
while True:
    ...
    url = "http://kaijiang.zhcw.com/zhcw/inc/ssq/ssq_wqhg.jsp?pageNum=" + str(which)
    soup = BeautifulSoup(urllib.request.urlopen(url).read(), "lxml")
    if which == 1:
        all_pages = int(((soup.find_all("p", attrs={"class": "pg"})[0]).find_all("strong")[0]).get_text())
