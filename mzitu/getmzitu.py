...
def download_onemm(mmhome_url): #下载一个图集
    ...
    mm_dir_name = re.sub(r'[\/:*?"<>|]', '_', mm_dir_name+'_'+mmhome_url.split('/')[-1]) #目录名中包含有特殊字符"\ / : * ? " < > |"，替换为下划线
    count = 0
    ...
    for k in range(1,int(page)+1,1):
        if os.access('images/'+mm_dir_name+'/'+str(k)+'.jpg', os.F_OK):
            ...
            continue
        if int(page) >= 100:
            new_img_url = first_img_url[:-7] + '{0:03d}'.format(k) + first_img_url[-4:]
        else:
        ...
        if new_img_url and (new_img_url.startswith('http://') or new_img_url.startswith('https://')):
            ...
            print(mmhome_url + "/" + str(k) + " of '" + mm_dir_name + "' 无法下载,新")
        if k == int(page):
            return 0
    ...
    for i in range(k,int(page)+1,1):
        ...
        if new_img_url and (new_img_url.startswith('http://') or new_img_url.startswith('https://')):
            ...
            print(mmhome_url + "/" + str(i) + " of '" + mm_dir_name + "' 无法下载,旧")
...
def download():
    ...
    soup = BeautifulSoup(html, 'lxml')
    all_pages = int((soup.find_all("a", attrs={"class": "page-numbers"}))[3].contents[1]) #获取总概览页面数
